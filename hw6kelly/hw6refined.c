...
struct clock_thread_parameter
{
  ...
  int                 period;
  struct pause_flag * control_pause;
  struct pause_flag * data_pause;
};
...
int main( void )
{
  ...
  if (io != NULL)
  {
    ...
    thread_motor_parameter.A_PWM_pin = 12;  // Make into defines?
    ...
    thread_motor_parameter.mode = MODE_1;   // can, should be local
