...
void RedBlink(int count, volatile struct io_peripherals *io)
{
  int i = 0;
  
  while(i < count)
  {
    GPIO_SET( &(io->gpio), 12);  // Red LED on, GPIO12
    usleep(100*1000);            // numbers in micro seconds (100msec.)
    GPIO_CLR( &(io->gpio), 12);  // Red LED off, GPIO12
    usleep(100*1000);            // numbers in micro seconds (100msec.)
    ++i;
  }
}
...
void GreenBlink(int count, volatile struct io_peripherals *io)
{
  int i = 0;
  
  while(i < count)
  {
    GPIO_SET( &(io->gpio), 13);  // Green LED on, GPIO13
    usleep(100*1000);            // numbers in micro seconds (100msec.)
    GPIO_CLR( &(io->gpio), 13);  // Green LED off, GPIO13
    usleep(100*1000);            // numbers in micro seconds (100msec.)
    ++i;
  }
}
