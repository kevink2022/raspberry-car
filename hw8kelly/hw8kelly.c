  ...
  while (!(parameter->done->done))
  {
    ...
    if(parameter->camera_signal->recording){
      ...
      if (diverge_point < 3){
            ...
            printf("\n**********SLOW**********\n");
            local_pin_values.A_PWM = PWM_MOTOR_MAX - (DIVERGE_CUTOFF - diverge_point);//+ (diverge_point/2)*10;
            local_pin_values.B_PWM = PWM_MOTOR_MAX - (DIVERGE_CUTOFF - diverge_point);//+ (diverge_point/2)*10;
            local_pin_values.AI1 = 0;
            local_pin_values.AI2 = 1;
            local_pin_values.BI1 = 0;
            local_pin_values.BI2 = 1;
